...
def plot_standalone_vs_preconditioned(
    ...
    Uses two side-by-side subplots so each method has its own honest
    x-axis — standalone shows all 500 V-cycle iterations, preconditioned
    shows only its actual outer iterations (e.g. 6).
    ...
    tol : float, optional
        Tolerance line drawn on both subplots.
    save_path : str, optional
        Base path (no extension).  PDF + PNG saved if provided.
    """
    ...
    if tol is not None:
        ...
        ax_left.text(1, tol * 1.4, f"tol = {tol:.0e}",
                     fontsize=8.5, color="#666666")
    status_sa = "Converged" if result_standalone.converged else "Did Not Converge (DNF)"
    ax_left.set_title(
    ...
    if tol is not None:
        ...
        ax_right.text(0.05, tol * 1.4, f"tol = {tol:.0e}",
                      fontsize=8.5, color="#666666")
